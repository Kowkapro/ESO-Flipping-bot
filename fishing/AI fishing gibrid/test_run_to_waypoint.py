"""
Test 4: Map → Waypoint → Turn → RUN to waypoint with live steering

Full cycle:
1. Open map, zoom, YOLO find nearest blue_hook, set waypoint
2. Close map, turn to face waypoint
3. Sprint toward waypoint, continuously correcting course via compass_marker
4. Stop when waypoint marker disappears (arrived) or F6

While running, the bot watches the compass_marker every ~150ms.
...
def steer_smooth(total_dx):
    """Lightweight steering correction while running — fast and smooth.

    Shorter than human_mouse_arc, designed for mid-run adjustments.
    Duration: 0.05-0.15s (quick nudge).
    """
    abs_dx = abs(total_dx)
    if abs_dx == 0:
        return
...
# Running (Phase C)
RUN_STEER_DAMPING = 0.7        # gentler while running (avoid overcorrection)
RUN_STEER_MAX_PX = 1500        # max correction per tick while running
RUN_DEAD_ZONE_FRAC = 0.03      # wider dead zone while running
RUN_MAX_DURATION = 60.0         # max run time in seconds (safety)
RUN_DETECT_INTERVAL = 0.15     # YOLO check every 150ms while running
MARKER_LOST_THRESHOLD = 8      # stop after N consecutive frames without marker

# Stuck detection
STUCK_CHECK_INTERVAL = 2.0     # check every 2 seconds if marker moved
STUCK_MIN_CHANGE_PX = 15       # marker must move at least 15px in 2s
STUCK_JUMP_COUNT = 3            # jump 2-3 times when stuck
...
def phase_c_run_to_waypoint(model, sct, monitor, screen_cx, stop_flag):
    """Sprint toward waypoint with continuous compass steering."""
    print()
    print("=" * 40)
    ...
    run_dead_zone = screen_cx * 2 * RUN_DEAD_ZONE_FRAC  # ~58px for 1920
    marker_lost_count = 0
    steer_count = 0
    jump_count = 0
        ...
        while not stop_flag[0]:
            ...
            if not markers:
                ...
                if marker_lost_count >= MARKER_LOST_THRESHOLD:
                    print(f"\n[C] Marker lost for {marker_lost_count} frames — arrived or passed!")
                    break
            ...
            if now - last_stuck_check_time >= STUCK_CHECK_INTERVAL:
                if last_marker_x is not None:
                    ...
                    if marker_change < STUCK_MIN_CHANGE_PX:
                        # Stuck! Jump to get unstuck
                        jumps = random.randint(2, STUCK_JUMP_COUNT)
                        jump_count += jumps
                        print(f"  [{elapsed:.1f}s] STUCK! Marker moved only {marker_change:.0f}px "
                              f"in {STUCK_CHECK_INTERVAL}s — jumping {jumps}x")
    ...
    print(f"[C] Ran for {total_time:.1f}s, {steer_count} steering checks, {jump_count} jumps")
...
def main():
    ...
    print("=" * 60)
    print("  TEST 4: Map → Waypoint → Turn → RUN to it")
    print("=" * 60)
    print("  F5 — Run test (switch to ESO first!)")
    print("  F6 — Stop immediately (releases keys)")
    print("=" * 60)
    ...
    # Phase C: Run!
    phase_c_run_to_waypoint(model, sct, monitor, screen_cx, stop_flag)

    print()
    print("=" * 60)
    print("[TEST 4] DONE!")
    print("  Check in-game:")
    print("  - Did the bot reach the fishing hole?")
    print("  - Was the path smooth?")
    print("  - Did steering keep the waypoint centered?")
    print("=" * 60)
